    ...
    <form v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      ...
      <div v-if="user.role !== 'admin'">
        ...
        <select v-model="form.role" class="input">
          ...
          <option value="account_manager">{{ t('admin.users.roles.account_manager') }}</option>
        </select>
      </div>
      <div>
...
watch(() => props.user, (u) => {
  if (u) {
    Object.assign(form, { email: u.email, password: '', username: u.username || '', notes: u.notes || '', role: u.role === 'account_manager' ? 'account_manager' : 'user', concurrency: u.concurrency, rpm_limit: u.rpm_limit ?? 0, customAttributes: {} })
