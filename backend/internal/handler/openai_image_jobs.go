...
const (
	...
	defaultOpenAIImageJobQueueSize     = 256
)
...
func (h *OpenAIGatewayHandler) ServeImageJobPublicFile(c *gin.Context) {
	...
	if len(parts) < 3 || parts[0] != "image_jobs" || !isSafeImageJobID(parts[1]) || parts[2] != "public" {
		c.JSON(http.StatusNotFound, gin.H{
			"error": gin.H{
				"type":    "not_found_error",
...
func (h *OpenAIGatewayHandler) createImageJob(c *gin.Context, endpoint string) {
	...
	if strings.TrimSpace(header.Get("X-Forwarded-Host")) == "" && c.Request != nil && strings.TrimSpace(c.Request.Host) != "" {
	...
	go h.runImageJob(job.ID, endpoint, body, header)
