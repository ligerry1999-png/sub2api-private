		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.mode == config.RunModeSimple {
				if tt.seed {
					...
					// The first count checks whether this is a fresh database; the
					// remaining five counts cover each platform's default groups.
					mock.ExpectQuery(`SELECT COUNT\("groups"\."id"\) FROM "groups"$`).
						WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(0))
					for i := 0; i < 5; i++ {
						mock.ExpectQuery(`SELECT COUNT\("groups"\."id"\) FROM "groups" WHERE`).
