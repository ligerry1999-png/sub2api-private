...
// Some ChatGPT Responses revisions reject the explicit image_generation tool
// choice while accepting the same tool with automatic selection. Keep the
// explicit choice by default and use this narrow form only after that exact
// compatibility error.
...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	if s.shouldTryOpenAIImagesWorker(ctx, parsed) {
...
func (s *OpenAIGatewayService) shouldTryOpenAIImagesWorker(ctx context.Context, parsed *OpenAIImagesRequest) bool {
	return s.openAIImagesWorkerEnabled(parsed) && !OpenAIImageWorkerFallbackAllowedFromContext(ctx)
}

const (
	openAIImagesOAuthUnavailableDefaultCooldown = 45 * time.Second
...
// openAIImagesOAuthUnavailableCooldown (capped at 45 seconds).
//
...
// happens on healthy accounts. Writing a long account-level cooldown from it
...
func (s *OpenAIGatewayService) coolOpenAIImagesOAuthTool(ctx context.Context, account *Account) {
	...
	if s.settingService != nil {
		...
		} else if settings != nil && settings.CooldownMinutes > 0 {
			cooldown = time.Duration(settings.CooldownMinutes) * time.Minute
		}
	}
	if cooldown > openAIImagesOAuthUnavailableDefaultCooldown {
		cooldown = openAIImagesOAuthUnavailableDefaultCooldown
	}
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthResponseError(
	...
	if upstreamErr.Code == "image_generation_unavailable" && shouldCoolOpenAIImagesToolForError(upstreamErr) {
