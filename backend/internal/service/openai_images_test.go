...
func TestBuildOpenAIImagesAPIResponse_FileURLDelivery(t *testing.T) {
	dataDir := t.TempDir()
	rawPNG, err := base64.StdEncoding.DecodeString("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=")
	require.NoError(t, err)
	imgB64 := base64.StdEncoding.EncodeToString(rawPNG)

	body, err := buildOpenAIImagesAPIResponseWithDelivery(
		[]openAIResponsesImageResult{{Result: imgB64, OutputFormat: "png"}},
		1710000000,
		nil,
		openAIResponsesImageResult{Model: "gpt-image-2"},
		"url",
		&openAIImagesResultDeliveryOptions{
	...
	require.True(t, strings.HasPrefix(url, "https://sub2api.example/image-files/image_jobs/imgjob_test/public/"))
	require.True(t, strings.HasSuffix(url, ".png"))
	require.False(t, strings.HasPrefix(url, "data:image/"))
	require.False(t, gjson.GetBytes(body, "data.0.delivery_fallback").Exists())
	...
	require.NoError(t, err)
	require.Equal(t, rawPNG, saved)
}

func TestBuildOpenAIImagesAPIResponse_FileURLDeliveryFallback(t *testing.T) {
	body, err := buildOpenAIImagesAPIResponseWithDelivery(
		[]openAIResponsesImageResult{{Result: "not-base64", OutputFormat: "png"}},
		1710000000,
		nil,
		openAIResponsesImageResult{Model: "gpt-image-2"},
		"url",
	...
	require.NoError(t, err)
	require.Equal(t, "data:image/png;base64,not-base64", gjson.GetBytes(body, "data.0.url").String())
	require.Equal(t, "base64", gjson.GetBytes(body, "data.0.delivery_fallback").String())
}
