...
func (s *GrokOAuthService) passwordAuthEnabled() bool {
	// Keep the legacy setting readable for configuration compatibility, but do
	// not accept raw account passwords. Grok accounts remain connectable through
	// browser OAuth, refresh_token, and one-time SSO exchange.
	return false
}
