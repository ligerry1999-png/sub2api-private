...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	// Strict opt-in: missing/empty stays disabled so GPT/Codex/Claude model IDs
	// can never be silently rewritten to Grok after an upgrade.
	result.GrokCrossClientModelMapEnabled = strings.EqualFold(
		strings.TrimSpace(settings[SettingKeyGrokCrossClientModelMapEnabled]),
		"true",
	)
