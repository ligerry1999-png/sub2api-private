...
const (
	...
	// 图片 worker 失败只做短暂换号，避免一次图片能力异常把账号停到
	// 30 分钟或更久；真正的长期配额由上游配额信息和管理员策略负责。
	openAIImageWorkerDefaultCooldown      = 45 * time.Second
	openAIImageWorkerMaxCooldown          = 45 * time.Second
...
func (s *OpenAIGatewayService) BlockOpenAIImageWorkerScheduling(account *Account, until time.Time) {
	...
	blockUntil = capOpenAIImageWorkerCooldown(now, blockUntil)
