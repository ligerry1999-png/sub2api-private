...
// A text-only model response is request-scoped. Retry the same healthy account
// before switching accounts, and never persist an account-level cooldown.
func TestOpenAIGatewayServiceForwardImages_TextFallbackRetriesBeforeSwitching(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &modelNotFoundAccountRepoStub{}
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat"}`)
	...
	require.Empty(t, repo.modelRateLimitCalls,
		"a text-only response must not create a 30-minute account cooldown")
}
