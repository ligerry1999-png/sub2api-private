...
# Sub2API v0.2.6 私有保留升级执行清单

> 开始日期：2026-09-18
>
> 线上基线：`0.2.4-private.1` / `75e62c5ee93443a94ab39c122030f3015530c14e`
>
> 官方目标：`v0.2.6` / `8b69738d782ccaa7fd26511e1cca26ba8d1b58db`

## 计划

- [ ] 审计官方 v0.2.5、v0.2.6 与私有 image-jobs、图片日志、Codex Images、部署保护的重叠文件
- [ ] 分阶段合入官方 v0.2.5、v0.2.6，解决冲突并保留私有行为并集
- [ ] 更新私有版本号、生成代码、迁移和必要文档
- [ ] 增加 Codex ticket 默认关闭、实时开关和敏感票据脱敏护栏
- [ ] 运行后端定向/全量测试、前端检查、lint 和漏洞扫描
- [ ] 运行无外网生产数据库升级、旧版回滚、恢复和再次升级演练
- [ ] 推送候选，确认精确 SHA 的 CI、安全扫描和数据库演练通过
- [ ] 仅通过 GitHub Actions 构建并部署，服务器不构建
- [ ] 部署后核对版本、健康、图片队列、私有路由、图片日志和计费
...
2026-09-18 v0.2.6-private.1 升级候选：已在隔离工作树合入官方 2.5/2.6，保留 `/v1/image-jobs/*`、图片日志、Codex Images 原生路由、透明背景参数、ChatGPT2API worker/fallback 及 GitHub Runner 部署保护。合并冲突集中在 Images 路由、Wire 注入和定价兜底，均按官方新逻辑与私有行为并集处理；强制 Responses 回退、原生 Images 端点不可用时的兼容回退、Image 2.5 计费兜底已保留。

本机验证：前端 `vue-tsc --noEmit` 通过；生产构建通过；全量 `300` 个测试文件、`2248` 条测试通过。一个旧测试仍检查已迁移的 `modelsListEndpoint` 文案，已同步到当前真实的 `model_allowlist` 候选流程，未添加无效兼容代码。后端本机没有 Go 工具链，等待 GitHub Actions 完成后端测试、Wire 生成校验和安全扫描。
